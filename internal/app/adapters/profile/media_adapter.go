// internal/app/adapters/profile/media_adapter.go
...
// MediaAdapter adapts the media module's Service to the profile domain's MediaService
type MediaAdapter struct {
	mediaSvc mediaService.Service
}

// NewMediaAdapter creates a new media adapter
...
func (a *MediaAdapter) UploadFile(ctx context.Context, cmd domain.UploadMediaCommand) (*domain.MediaInfo, error) {
	...
	// Convert media service response to domain MediaInfo
	return &domain.MediaInfo{
		ID:         media.ID,
		URL:        media.URL,
		MediaType:  media.MediaTypeID, // Media type ID
