...
type CasbinService interface {
	...
	// ReloadPolicies forces the enforcer to re-read its in-memory model
	// from the adapter (DB).
	//
	// Call this after writes that must be visible to subsequent permission
	// checks in the same request (e.g. accepting an invitation that grants
	// a new account role). The enforcer's auto-load runs on an interval;
	// without this, a just-written g rule may not be visible until the
	// next cycle.
