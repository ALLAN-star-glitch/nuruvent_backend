...
func (s *eventService) CreateDraft(ctx context.Context, cmd CreateDraftCommand) (*domain.Event, error) {
	// Determine institution ID (empty string for personal events)
	institutionID := ""
	if cmd.InstitutionID != nil {
		institutionID = *cmd.InstitutionID
	}

	log.Printf("🔄 CreateDraft called: Name='%s', TypeID='%s', InstitutionID='%s', TeamType='%s'",
		cmd.Name, cmd.EventTypeID, institutionID, cmd.TeamType)

	// ✅ Check permission based on team type
	if cmd.TeamType == "personal" {
		// Personal events: user can create their own events
		// No institution permission needed
		if cmd.CreatedBy == "" {
			return nil, errors.New("user ID is required for personal events")
		}
	} else {
		// Institution events: check if user has permission for this institution
		if institutionID == "" {
			return nil, errors.New("institution ID is required for institution events")
		}
		if !s.permChecker.CanManageEvent(ctx, cmd.CreatedBy, institutionID) {
			log.Printf("❌ Permission denied for user %s on institution %s", cmd.CreatedBy, institutionID)
			return nil, errors.New("insufficient permissions to create events for this institution")
		}
	}
	...
	if rawUserInput == "" {
		rawUserInput = "Untitled Event"
	}
	log.Printf("📝 User input: '%s'", rawUserInput)

	// 2. Display Name - PRESERVE USER INPUT
	...
	// 3. Name - Internal identifier (sanitized)
	...
	// ✅ Use domain.NewEvent with InstitutionID (empty string for personal)
...
func (s *eventService) CreateEvent(ctx context.Context, cmd CreateEventCommand) (*domain.Event, error) {
	...
	log.Printf("🔄 CreateEvent (published) called: Name='%s', TypeID='%s', InstitutionID='%s', TeamType='%s'",
		cmd.Name, cmd.EventTypeID, institutionID, cmd.TeamType)
...
func (s *eventService) UpdateEvent(ctx context.Context, cmd UpdateEventCommand) (*domain.Event, error) {
	...
	// ✅ Check permission using InstitutionID (can be empty for personal)
	if !s.permChecker.CanUpdateEvent(ctx, cmd.UpdatedBy, event.InstitutionID) {
...
func (s *eventService) DeleteEvent(ctx context.Context, id, deletedBy string) error {
	...
	// ✅ Check permission using InstitutionID
	if !s.permChecker.CanDeleteEvent(ctx, deletedBy, event.InstitutionID) {
...
func (s *eventService) RestoreEvent(ctx context.Context, id, restoredBy string) (*domain.Event, error) {
	...
	if !s.permChecker.CanUpdateEvent(ctx, restoredBy, event.InstitutionID) {
...
// Delete all events for an institution (soft delete)
...
// Permanently delete all events for an institution
func (s *eventService) PermanentlyDeleteEventsByInstitution(ctx context.Context, institutionID string, deletedBy string) (*BulkDeleteResult, error) {
	...
	// Get all events for this institution (including soft-deleted)
	events, _, err := s.repo.GetEventsByInstitution(ctx, institutionID, 0, 0)
	if err != nil {
		return nil, fmt.Errorf("failed to get events for institution: %w", err)
	}
...
func (s *eventService) PublishEvent(ctx context.Context, id, publishedBy string) (*domain.Event, error) {
	...
	if !s.permChecker.CanUpdateEvent(ctx, publishedBy, event.InstitutionID) {
...
func (s *eventService) CancelEvent(ctx context.Context, id, cancelledBy string) (*domain.Event, error) {
	...
	if !s.permChecker.CanUpdateEvent(ctx, cancelledBy, event.InstitutionID) {
...
func (s *eventService) BulkPublishEvents(ctx context.Context, ids []string, publishedBy string) (*BulkStatusResult, error) {
	...
	// ✅ If ALL events failed validation
...
func (s *eventService) DuplicateEvent(ctx context.Context, id string, cmd DuplicateEventCommand) (*domain.Event, error) {
	...
	if !s.permChecker.CanManageEvent(ctx, "", original.InstitutionID) {
...
func (s *eventService) UploadEventImage(ctx context.Context, cmd UploadEventImageCommand) (*domain.MediaInfo, error) {
	...
	if !s.permChecker.CanUpdateEvent(ctx, cmd.UploadedBy, event.InstitutionID) {
...
func (s *eventService) DeleteEventCertificate(ctx context.Context, eventID string, deletedBy string) error {
	...
	if !s.permChecker.CanUpdateEvent(ctx, deletedBy, event.InstitutionID) {
...
func (s *eventService) ListEvents(ctx context.Context, filters ListEventsFilters) ([]*domain.Event, int64, error) {
	fmt.Printf("DEBUG Service: OnlyDeleted=%v, InstitutionID=%s\n", filters.OnlyDeleted, filters.InstitutionID)
...
func (s *eventService) GetEventsByInstitution(ctx context.Context, institutionID string, page, pageSize int) ([]*domain.Event, int64, error) {
	...
	limit := pageSize
	offset := (page - 1) * pageSize
	return s.repo.GetEventsByInstitution(ctx, institutionID, limit, offset)
}
...
// GetEventsByUser returns events created by a specific user
...
func (s *eventService) GetEventsByUserWithCreator(ctx context.Context, userID string, page, pageSize int) ([]*domain.Event, int64, error) {
	...
	return s.repo.GetEventsByUserWithCreator(ctx, userID, limit, offset)
}
...
func (s *eventService) GetEventsByInstitutionWithCreator(ctx context.Context, institutionID string, page, pageSize int) ([]*domain.Event, int64, error) {
	...
	return s.repo.GetEventsByInstitutionWithCreator(ctx, institutionID, limit, offset)
}